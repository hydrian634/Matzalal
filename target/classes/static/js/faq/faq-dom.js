window.addEventListener("load", function(){
	...
	let content = document.querySelector(".faq-template"); //꽂아줄 부분 상위 범위
	...
	searchBtn.onclick = async function(e) {
		...
		//컨트롤러로 보내주기.
		let response = await fetch(`/api/faqs?q=${query}`);
	...
	function bind(list){	
        ...
        for (let f of list){	
			let formattedDate = formatDate(f.createdDate);
			
		let template = `
	       <section class="faq-list-box-left">
                    <h1 class="d:none">faq 목록</h1>
                    <div class="list-left-side">
                        <span class="faq-number">${f.faqId}</span><span>.</span>
                        <h1>${f.title}</h1>
